...
- **Recipient search.** As you type, the wallet matches local contacts instantly and runs a debounced (~0.4 s) network lookup in parallel. Results render as tappable cards:
  - A **name / @handle** resolves via [NIP-05](name-authority.md) → a verified card (`name@domain`).
...
**Requests** reuse the surface: choose *Request* instead of *Pay* to issue an Invoice-1 to a contact (or broadcast a "requesting X ツ" code). Incoming requests appear as approve/decline cards; see [Cancel & decline](cancel-decline.md).
