...
- **Resolution.** A wallet resolving `alice@goblin.st` fetches `https://goblin.st/.well-known/nostr.json?name=alice` (over [Tor](../pillars/nym-http.md), at the authority's pinned `.onion`) and reads the pubkey (and any relay hints). A **reverse** lookup (name-by-pubkey) lets a wallet show the `name` for a key it only knows by `npub`.
