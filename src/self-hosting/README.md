...
> **Summary.** Goblin's public infrastructure (the `goblin.st` name authority, the default `relay.floonet.dev` relay, and its mixnet exits) is all run-your-own. None of it is a hard dependency: you can point a Goblin wallet at your own name authority, your own relay, and your own mixnet exit, and build the app from source.
...
| **Mixnet exit** | The mixnet exit(s) Goblin egresses through | [Run a mixnet exit](nym-requester.md) |
...
- **Mixnet exit**: prefer your own public exit with the `GOBLIN_NYM_IPR` environment variable (see [Run a mixnet exit](nym-requester.md)). A relay's co-located [scoped exit](../pillars/nym-exit.md) is advertised through the relay pool rather than configured per wallet.
