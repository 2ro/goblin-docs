...
- **Payment retention.** The point of a payment relay is to hold a gift-wrap until its recipient comes online, so don't set an aggressive expiry or eviction policy that could drop an unfetched payment. The project's `relay.floonet.dev` **guarantees** payment-message retention (it cannot prematurely delete a payment), which is why it is the one unremovable relay pinned in every wallet; run yours the same way.
