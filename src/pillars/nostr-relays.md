...
The pool is a maintained list of vetted public relays, fetched from the project's published pool file **over the mixnet**, cached on disk, and refreshed when the cache is older than 7 days. A pinned copy is compiled into the app, byte-for-byte the published contents, so first-run and offline behave exactly like a fresh fetch.
...
- `goblin/src/nostr/pool.rs`: `RelayPool` / `PoolRelay` (the `exit` field), `PINNED_POOL`, `load()` / `refresh_if_stale()`, the NIP-11 gate (`nip11_pass()`, `probe()`), `weighted_order()`, `usable_discovery_relays()`, `MIN_MESSAGE_LENGTH = 131072`.
