...
- **Dialing.** The [relay transport](tor-relay-transport.md) hands the relay's hostname to arti, which builds a Tor circuit out through an exit relay to that host; the usual TLS + websocket handshake then runs over the resulting stream.
...
- The client that dials the relay: [The embedded Tor client](tor-client.md).
