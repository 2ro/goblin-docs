...
> **Summary.** Every HTTP request Goblin makes (NIP-05 name resolution and registration, the price feed, avatar fetches, the relay pool and its NIP-11 probes) goes through [Tor](nym-client.md) via a Tor-exit circuit to the destination's ordinary clearnet host. TLS is validated against the hostname, and HTTP/1.1 runs on top. There is no clearnet HTTP path from the device, and no onion hop for any of it.
...
- Why the wallet needs no DNS resolver: [Name resolution under Tor](nym-dns.md).
